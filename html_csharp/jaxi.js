...
function beginCompile () {
	...
	loadJS('http://jsil.org/try/Libraries/JSIL.js');
  ...
  $.ajax({
    ...
    type: 'POST',
    url: "http://52.25.194.116/try2/compile.aspx",
    data: sourceCode,
...
runScript = function (args) {
  try {
  alert(args.javascript);
  	alert(args.entryPoint);
  
    eval.call(this, args.javascript);

	alert($asm00.constructor.name);
    JSIL.Initialize();
...
function runInOutputWindow (javascript, entryPoint, warnings) {
  ...
  //var outputDoc = document.getElementById("iframe").contentDocument;
	var outputWindow = document.getElementById("codeCruncher");
    //var outputDoc = document.getElementById("iframe").contentDocument;
